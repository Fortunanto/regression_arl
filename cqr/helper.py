...
def train_icp(nc, X_train, y_train, idx_train, idx_cal, significance, condition=None):
...
class AllQNet_RegressorAdapter(RegressorAdapter):
    ...
    def __init__(self,
        ...
        self.model = torch_models.all_q_model(quantiles=self.all_quantiles,
                                              ...
                                              dropout=dropout, out_size = out_size)
...
class QuantileForestRegressorAdapter(RegressorAdapter):
    ...
    def predict(self, x):
        ...
        """
        self.cv_quantiles = self.cv_quantiles/100
        lower = self.rfqr.predict(x, quantiles=self.cv_quantiles[0])
        upper = self.rfqr.predict(x, quantiles=self.cv_quantiles[1])
