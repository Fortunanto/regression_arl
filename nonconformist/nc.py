...
class QuantileRegErrFunc(RegressionErrFunc):
    ...
    def apply(self, prediction, y):
        y_lower = prediction[:,0:2]
        y_upper = prediction[:,2:4]
        error_low = y_lower - y
...
class RegressorNc(BaseModelNc):
	...
	def predict(self, x, nc, significance=None):
		...
		if significance:
			intervals = np.zeros((x.shape[0], 2*x.shape[1]))
			err_dist = self.err_func.apply_inverse(nc, significance)[0]
			err_dist = np.vstack([err_dist] * n_test)
			if prediction.ndim > 1: # CQR
				intervals[:, 0:2] = prediction[:,0:2] - err_dist
				intervals[:, 2:] = prediction[:,2:] + err_dist
