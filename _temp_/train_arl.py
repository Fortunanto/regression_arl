...
def train_model(
    ...
    while train:
        with tqdm(train_loader, desc="Training", ncols=100) as progress_bar:

            for step, (train_img, train_target) in enumerate(progress_bar):
                ...
                if optimizer_adv:
                    # assert False, f"optimizer_adv {optimizer_adv} total_steps {total_steps} pretrain_steps {pretrain_steps}"
                    if total_steps >= pretrain_steps:
                        loss_adv, logging_dict = model.adversary_step(
                            train_img, train_logits, train_target
                        )
                ...
                postfix_dict = {
                    "Adv Loss": "{:.4f}".format(loss_adv),
                    "Learner Loss": "{:.4f}".format(loss_learner),
                }
...
def main(args):
    ...
    # img_folder, metadata_file,split="train"
    img_folder = "fairface"
    train_dataset = CustomDataset(img_folder=img_folder, metadata_file="fairface_label_train.csv",split="train")
    test_dataset = CustomDataset(img_folder=img_folder, metadata_file="fairface_label_val.csv",split="val")
